...
# Get a basemap
shef_basemap <- get_map(location = shef_bbox, 
                   zoom = 11,
...
![](index_files/figure-gfm/school-features-1.png)<!-- --> For the
purpose of comparing isochrones created from different network
...
park_bbox <- st_buffer(sf_park, 1000) %>% 
  st_bbox() %>%
  as.numeric()

# Get a basemap
park_basemap <- get_map(location = park_bbox, 
                        zoom = 14, 
                        maptype = 'terrain-background',
                        source = 'stamen')
